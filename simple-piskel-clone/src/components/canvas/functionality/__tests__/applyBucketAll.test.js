...
test('applyBucketAll throws when mouse one of the arguments not specified', () => {
  ...
  expect(() => applyBucketAll({ colorToApply: '#123456', currMousePosition: { x: 1, y: 2 } })).toThrow();
});

test('applyBucketAll works correctly', () => {
  expect(() => applyBucketAll({ ctx, colorToApply: '#123456', currMousePosition: { x: 1, y: 2 } })).not.toThrow();
});
