...
test('applyBucket throws when mouse one of the arguments not specified', () => {
  ...
  expect(() => applyBucket({ colorToApply: '#123456', currMousePosition: { x: 1, y: 2 } })).toThrow();
});

test('applyBucket works correctly', () => {
  expect(() => applyBucket({ ctx, colorToApply: '#123456', currMousePosition: { x: 1, y: 2 } })).not.toThrow();
});
